###############################################################
# Tax 
# Andrew Goldsborough - 08/12/2015
# calculate tax
###############################################################

#Input gross wage
println("Input annual salary in GBP:");
salary = readline(STDIN);
salary = parse(Float64,chomp(salary));
...
if salary > NIC_threshold 
    if salary > NIC_upper
        ...
        NIC = (NIC_upper - NIC_threshold)*0.12 + (salary - NIC_threshold)*0.02;
    else
...
println();
println("Gross pay = £",salary);
println("National insurance contributions = £",NIC);
println("Tax = £",tax);
println("Take home pay = £", round(salary - NIC - tax,2));
